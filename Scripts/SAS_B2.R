...
# Exposure: Hospitalized COVID-19 (B2) - South Asian
exposure_raw <- fread("./COVID-19_AMI_All/COVID19_HGI_B2_ALL_sas_leave23andme_20220403.tsv.gz")
...
exposure_c1$exposure <- "Hospitalized_COVID19_B2_SAS"
exposure_c1$id.exposure <- "covid_b2_sas"
...
write.csv(mr_dat, "./SAS_B2_mr_dat.csv", row.names = FALSE)
...
write.csv(odd, "./SAS_B2_results.csv", row.names = FALSE)
...
ggsave("./SAS_B2_scatter.png", mr_scatter_plot(res, mr_dat)[[1]], width = 8, height = 6)
ggsave("./SAS_B2_forest.png", mr_forest_plot(singlesnp_res)[[1]], width = 10, height = 8)
ggsave("./SAS_B2_leaveoneout.png", mr_leaveoneout_plot(mr_leaveoneout(mr_dat))[[1]], width = 10, height = 8)
ggsave("./SAS_B2_funnel.png", mr_funnel_plot(singlesnp_res)[[1]], width = 8, height = 6)

# ==================== Sensitivity Tests ====================
mr_heterogeneity(mr_dat)
mr_pleiotropy_test(mr_dat)

cat("SAS B2 Analysis Completed Successfully\n")
