# ================================================
# EUR - Very Severe COVID-19 (B2) vs AMI
# ================================================
...
exposure_snps <- ld_clump(dat = exposure_c1, clump_kb = 10000, clump_r2 = 0.001,
                          plink_bin = genetics.binaRies::get_plink_binary(), bfile = "./EUR")

# Outcome
outcome_raw <- fread("./COVID-19_AMI_All/GCST90473536.h.tsv.gz")
...
ggsave("results/figures/EUR_C2_forest.png", mr_forest_plot(mr_singlesnp(mr_dat))[[1]], width=10, height=8)
ggsave("results/figures/EUR_C2_leaveoneout.png", mr_leaveoneout_plot(mr_leaveoneout(mr_dat))[[1]], width=10, height=8)
ggsave("results/figures/EUR_C2_funnel.png", mr_funnel_plot(mr_singlesnp(mr_dat))[[1]], width=8, height=6)

mr_heterogeneity(mr_dat)
mr_pleiotropy_test(mr_dat)

cat("EUR C2 Completed\n")
